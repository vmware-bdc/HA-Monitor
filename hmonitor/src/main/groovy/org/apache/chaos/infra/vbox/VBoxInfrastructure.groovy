...
class VBoxInfrastructure extends Infrastructure {

  @Override
  boolean PowerOffVM(VMReference vmRef) {
    VBoxManage manage = VBoxManage.controlVM(vmRef.name, VBoxManage.poweroff)
    Process ps = manage.start()
    ps.waitFor() == 0
  }

  @Override
  VMReference locate(String vmName) {
    VMReference vm = super.locate(vmName)
    vm['uuid'] = vmName
    vm
  }
}
